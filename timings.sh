...
for i in `seq 1 6`; do
    q.rb -r -y 0-49 -j hg-cvs-$i ./time-hg.sh 1000 $i 1
    q.rb -r -y 0-49 -j meth-cvs-$i ./time-meth1.R --args N=1000 NCV=$i NSIM=1
done
...
sacct --name $jobs --format=JobID,JobName%50,State,TotalCPU,UserCPU | grep cvs > timings/cvs.txt




## how does number of cases/controls change things?
for i in 1000 2000 4000 8000 32000 64000; do
